...
*   **Headless Multi-View Combined Preview**: **NEW!** Multi-view CAD renders (3/4 perspective, front, right, top) are composited into a single clear high-resolution grid with viewport labels and dividers. Avoids ComfyUI's sequential frame loading lag (`loading 1/4... 2/4...`), instantly presenting all combined objects in one clear view. Includes the dedicated **AI Headless Preview (Combined Views)** node.
...
| **AI Headless Preview (Combined Views)** | Composites multi-view headless renders into a single clear CAD-style grid with viewport labels and dividers. Direct UI display with instant loading (no multi-file carousel lag); connects to builder nodes or loads directly from an AI Scene Session. |
